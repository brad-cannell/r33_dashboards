...
go_uth_link_api <- function(slug){
  ...
  # returns a non-200). Return NULL so the pipeline can fall back to the most
  # recent saved link data instead of halting the whole build.
  if (httr::status_code(details) != 200) {
    warning(sprintf(
      "GO-UTHealth API returned HTTP %s for slug '%s'; skipping live link pull.",
  ...
  # Response parsed but no traffic payload (e.g. auth ok, empty result).
  if (is.null(parsed_json$traffic)) {
    warning(sprintf("GO-UTHealth returned no traffic for slug '%s'; skipping.", slug))
    return(NULL)
  }
...
if (any(vapply(link_list, is.null, logical(1)))) {
  # One or more GO-UTHealth link pulls failed (e.g. expired token). Fall back to
  # the most recent saved link data so the rest of the pipeline still completes.
  # Flag it so we don't overwrite the good saved copy with stale/empty data.
  link_hits_fresh <- FALSE
  existing_link_path <- here::here("data", "detect_tool", "detect_tool_link_hits.RDS")
  if (file.exists(existing_link_path)) {
    message("GO-UTHealth link data unavailable; reusing existing detect_tool_link_hits.RDS")
    link_hits <- readr::read_rds(existing_link_path)
  } else {
    message("GO-UTHealth link data unavailable and no saved copy found; using empty link data")
    link_hits <- data.frame(date = as.Date(character()), link = character(), hits = numeric())
  }
} else {
...
if (link_hits_fresh) {
  ...
  message("Skipping overwrite of detect_tool_link_hits.RDS (no fresh GO-UTHealth data).")
}
```
