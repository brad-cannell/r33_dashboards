...
free_text_fields <- c(
    ...
    "ri_em_reason", # suspect_em = 1 AND ri_necessities != 1 OR ri_environment != 1 OR ri_sedated != 1 OR ri_isolated != 1 OR ri_anxious != 1 OR ri_prohibited != 1 OR ri_unmet_needs != 1 OR ri_injuries != 1
...
answers_per_record <- map_vec(free_text_fields, ~ perc_answered(data, .x))
answers_per_eligible_record <- map_vec(free_text_fields, ~ perc_answer_based_on_availability(data, .x, free_text_branching_logic))

names(answers_per_record) <- free_text_fields
names(answers_per_eligible_record) <- free_text_fields

answers_per_record <- tibble::enframe(answers_per_record, name = "Textbox Variable", value = "Answered %")
answers_per_eligible_record <- tibble::enframe(answers_per_eligible_record, name = "Textbox Variable", value = "Answered %")
```
...
combined_answers <- left_join(answers_per_record, answers_per_eligible_record, by = "Textbox Variable", suffix = c(" per record", " per eligible record")) |>
    tidyr::pivot_longer(cols = c("Answered % per record", "Answered % per eligible record"), names_to = "Type", values_to = "Percent")

p1 <- ggplot(combined_answers, aes(x = Percent, y = `Textbox Variable`, fill = Type)) +
    geom_col(position = "dodge") + 
    scale_fill_manual(values = c("Answered % per record" = "#219ebc", "Answered % per eligible record" = "#023047"))
